...
def SVMTraining():
    ...
    ,["Question"]*np.shape(QuestionMark)[0]
    ,["Square"]*np.shape(Square)[0]
    ,["1_hor"]*np.shape(Horizontal[0])[0]
    ,["2_hor"]*np.shape(Horizontal[1])[0]
    ,["3_hor"]*np.shape(Horizontal[2])[0]
    ,["4_hor"]*np.shape(Horizontal[3])[0]
    # ,["5_hor"]*np.shape(Horizontal[4])[0]
    ,["1_ver"]*np.shape(Vertical[0])[0]
    ,["2_ver"]*np.shape(Vertical[1])[0]
    ,["3_ver"]*np.shape(Vertical[2])[0]
    ,["4_ver"]*np.shape(Vertical[3])[0]
    ,["5_ver"]*np.shape(Vertical[4])[0]
    ), axis=0)
